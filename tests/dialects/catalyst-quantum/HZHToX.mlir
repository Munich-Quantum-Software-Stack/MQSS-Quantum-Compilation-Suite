...
// RUN: %mqss-catalyst-opt %s --CommonReductionPass=mode=HZHToX --canonicalize  2>&1 | FileCheck %s

module @circuit_HZHToX {
  func.func public @jit_circuit_HZHToX() -> (tensor<4xi64>, tensor<4xi64>) attributes {llvm.emit_c_interface} {
    %0:2 = call @circuit_HZHToX_0() : () -> (tensor<4xi64>, tensor<4xi64>)
    return %0#0, %0#1 : tensor<4xi64>, tensor<4xi64>
  }
  func.func public @circuit_HZHToX_0() -> (tensor<4xi64>, tensor<4xi64>) attributes {diff_method = "parameter-shift", llvm.linkage = #llvm.linkage<internal>, qnode} {
    ...
    %out_qubits_3:2 = quantum.custom "CZ"() %out_qubits_1, %out_qubits_2 : !quantum.bit, !quantum.bit
    %out_qubits_4 = quantum.custom "Hadamard"() %out_qubits_3#1 : !quantum.bit
    %3 = quantum.insert %0[ 0], %out_qubits_3#0 : !quantum.reg, !quantum.bit
    %4 = quantum.insert %3[ 1], %out_qubits_4 : !quantum.reg, !quantum.bit
...
// CHECK: %out_qubits_1:2 = quantum.custom "CZ"() %out_qubits, %out_qubits_0 : !quantum.bit, !quantum.bit
