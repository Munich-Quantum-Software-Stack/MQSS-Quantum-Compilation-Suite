...
// RUN: %mqss-catalyst-opt %s --CommonReductionPass=mode=SAdjZToS --canonicalize  2>&1 | FileCheck %s

module @circuit_SAdjZToS {
  func.func public @jit_circuit_SAdjZToS() -> (tensor<4xi64>, tensor<4xi64>) attributes {llvm.emit_c_interface} {
    %0:2 = call @circuit_SAdjZToS_0() : () -> (tensor<4xi64>, tensor<4xi64>)
    return %0#0, %0#1 : tensor<4xi64>, tensor<4xi64>
  }
  func.func public @circuit_SAdjZToS_0() -> (tensor<4xi64>, tensor<4xi64>) attributes {diff_method = "parameter-shift", llvm.linkage = #llvm.linkage<internal>, qnode} {
    ...
    %out_qubits = quantum.custom "S"() %1 adj : !quantum.bit
    %out_qubits_0 = quantum.custom "PauliZ"() %out_qubits : !quantum.bit
    %2 = quantum.extract %0[ 1] : !quantum.reg -> !quantum.bit
    %out_qubits_1 = quantum.custom "S"() %2 : !quantum.bit
    %3 = quantum.insert %0[ 0], %out_qubits_0 : !quantum.reg, !quantum.bit
    %4 = quantum.insert %3[ 1], %out_qubits_1 : !quantum.reg, !quantum.bit
...
// CHECK: %out_qubits_0 = quantum.custom "S"() %2 : !quantum.bit
