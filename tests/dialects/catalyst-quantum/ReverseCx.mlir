...
// RUN: %mqss-catalyst-opt %s --CommonDecompositionPass=mode=ReverseCx  2>&1 | FileCheck %s

module @circuit_ReverseCx {
  func.func public @jit_circuit_ReverseCx() -> (tensor<4xi64>, tensor<4xi64>) attributes {llvm.emit_c_interface} {
    %0:2 = call @circuit_ReverseCx_0() : () -> (tensor<4xi64>, tensor<4xi64>)
    return %0#0, %0#1 : tensor<4xi64>, tensor<4xi64>
  }
  func.func public @circuit_ReverseCx_0() -> (tensor<4xi64>, tensor<4xi64>) attributes {diff_method = "parameter-shift", llvm.linkage = #llvm.linkage<internal>, qnode} {
    ...
    %out_qubits_0 = quantum.custom "PauliZ"() %2 : !quantum.bit
    ...
    %3 = quantum.insert %0[ 1], %out_qubits_2 : !quantum.reg, !quantum.bit
    %4 = quantum.insert %3[ 0], %out_qubits_3 : !quantum.reg, !quantum.bit
...
// CHECK: %out_qubits_2 = quantum.custom "H"() %out_qubits : !quantum.bit
// CHECK: %out_qubits_3:2 = quantum.custom "CNOT"() %out_qubits_1, %out_qubits_2 : !quantum.bit, !quantum.bit
// CHECK: %out_qubits_4 = quantum.custom "H"() %out_qubits_3#0 : !quantum.bit
// CHECK: %out_qubits_5 = quantum.custom "H"() %out_qubits_3#1 : !quantum.bit
// CHECK: %out_qubits_6 = quantum.custom "PauliY"() %out_qubits_4 : !quantum.bit
// CHECK: %out_qubits_7 = quantum.custom "PauliY"() %out_qubits_5 : !quantum.bit