...
// RUN: %mqss-catalyst-opt %s --CommonCommutePass=mode=X-CX 2>&1 | FileCheck %s

module @circuit_CommuteXCNot {
  func.func public @jit_circuit_CommuteXCNot() -> (tensor<4xi64>, tensor<4xi64>) attributes {llvm.emit_c_interface} {
    %0:2 = call @circuit_CommuteXCNot_0() : () -> (tensor<4xi64>, tensor<4xi64>)
    return %0#0, %0#1 : tensor<4xi64>, tensor<4xi64>
  }
  func.func public @circuit_CommuteXCNot_0() -> (tensor<4xi64>, tensor<4xi64>) attributes {diff_method = "parameter-shift", llvm.linkage = #llvm.linkage<internal>, qnode} {
    ...
    %out_qubits_1 = quantum.custom "PauliX"() %out_qubits_0#1 : !quantum.bit
    %out_qubits_2:2 = quantum.custom "CNOT"() %out_qubits_1, %out_qubits_0#0 : !quantum.bit, !quantum.bit
    %3 = quantum.insert %0[ 1], %out_qubits_2#0 : !quantum.reg, !quantum.bit
    %4 = quantum.insert %3[ 0], %out_qubits_2#1 : !quantum.reg, !quantum.bit
...
// CHECK: %out_qubits_0 = quantum.custom "PauliX"() %out_qubits#1 : !quantum.bit
// CHECK: %out_qubits_1 = quantum.custom "PauliX"() %out_qubits#1 : !quantum.bit
// CHECK: %out_qubits_2:2 = quantum.custom "CNOT"() %out_qubits_1, %out_qubits#0 : !quantum.bit, !quantum.bit
