...
#   Imported target: CATALYST::Quantum

option(CATALYST_AUTO_FETCH "Clone and build Catalyst's MLIR Quantum dialect from source if not found" OFF)
set(CATALYST_GIT_REPOSITORY "https://github.com/PennyLaneAI/catalyst.git"
...
# than guessing a tag string -- verify real tags yourself before pinning:
#   git ls-remote --tags https://github.com/PennyLaneAI/catalyst.git
set(CATALYST_GIT_TAG "v0.15.0" CACHE STRING "Catalyst git ref: tag, branch, or commit SHA")

set(CATALYST_BUILD_PARALLELISM "2" CACHE STRING
  ...
  if(NOT EXISTS "${_catalyst_marker}")
    ...
    if(NOT EXISTS "${_catalyst_src}/.git")
      execute_process(
        ...
        RESULT_VARIABLE _catalyst_rc ERROR_VARIABLE _catalyst_err)
    ...
    string(REPLACE "add_subdirectory(Driver)" "# add_subdirectory(Driver) -- patched out by FindCatalyst.cmake"
        _catalyst_lib_cmakelists_content "${_catalyst_lib_cmakelists_content}")
    file(WRITE "${_catalyst_lib_cmakelists}" "${_catalyst_lib_cmakelists_content}")

    # IMPORTANT: unlike cuda-quantum, there is no repo-root CMakeLists.txt.
    # The CMake project lives at mlir/ -- configure THAT directory, not the
    # clone root.
    execute_process(
      COMMAND ${CMAKE_COMMAND}
              -G ${CMAKE_GENERATOR}
              -S ${_catalyst_src}/mlir -B ${_catalyst_build}
              -DCMAKE_INSTALL_PREFIX=${_catalyst_install}
    ...
    execute_process(
      COMMAND ${CMAKE_COMMAND} --build ${_catalyst_build}
              --target MLIRMBQC MLIRQRef MLIRQuantum
              -- -j${CATALYST_BUILD_PARALLELISM}
      RESULT_VARIABLE _catalyst_rc)
    if(NOT _catalyst_rc EQUAL 0)
      message(FATAL_ERROR "FindCatalyst: MLIRQuantum build failed (exit ${_catalyst_rc})")
    endif()
    foreach(_hdr_dir Quantum QRef MBQC)
    file(COPY ${_catalyst_src}/mlir/include/${_hdr_dir} DESTINATION ${_catalyst_install}/include)
      if(EXISTS ${_catalyst_build}/include/${_hdr_dir})
        file(COPY ${_catalyst_build}/include/${_hdr_dir} DESTINATION ${_catalyst_install}/include)
      endif()
    endforeach()

  else()
    message(STATUS "FindCatalyst: reusing cached auto-build at ${_catalyst_install}")
  endif()
...
find_path(CATALYST_INCLUDE_DIR
...
find_package_handle_standard_args(Catalyst
  REQUIRED_VARS 
  CATALYST_INCLUDE_DIR 
  CATALYST_DIALECT_LIBRARY
  CATALYST_QREF_LIBRARY
  CATALYST_MBQC_LIBRARY
)
...
if(CATALYST_FOUND)
  ...
  if(NOT TARGET CATALYST::MBQC)
    ...
    set_target_properties(CATALYST::MBQC PROPERTIES
      IMPORTED_LOCATION "${CATALYST_MBQC_LIBRARY}"
      INTERFACE_INCLUDE_DIRECTORIES "${CATALYST_INCLUDE_DIRS}"
      # Best-guess transitive deps -- correct against real link errors,
  ...
  if(NOT TARGET CATALYST::QRef)
    ...
    set_target_properties(CATALYST::QRef PROPERTIES
      IMPORTED_LOCATION "${CATALYST_QREF_LIBRARY}"
      INTERFACE_INCLUDE_DIRECTORIES "${CATALYST_INCLUDE_DIRS}"
      # Best-guess transitive deps -- correct against real link errors,
      # same as CUDAQ::Quake/CUDAQ::CC earlier in this project.
      INTERFACE_LINK_LIBRARIES "CATALYST::MBQC;MLIRIR;MLIRSupport;MLIRFuncDialect")
  endif()
   if(NOT TARGET CATALYST::Quantum)
    add_library(CATALYST::Quantum UNKNOWN IMPORTED GLOBAL)
    set_target_properties(CATALYST::Quantum PROPERTIES
      IMPORTED_LOCATION "${CATALYST_DIALECT_LIBRARY}"
      INTERFACE_INCLUDE_DIRECTORIES "${CATALYST_INCLUDE_DIRS}"
      # Best-guess transitive deps -- correct against real link errors,
      # same as CUDAQ::Quake/CUDAQ::CC earlier in this project.
      INTERFACE_LINK_LIBRARIES "CATALYST::QRef;MLIRIR;MLIRSupport;MLIRFuncDialect")
  endif()
