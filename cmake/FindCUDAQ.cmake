...
# Off by default: cloning + building CUDA-Q is a multi-minute-to-hour
# operation and shouldn't happen silently on every fresh configure.
option(CUDAQ_AUTO_FETCH "Clone and build CUDA-Q from source if not found" OFF)
set(CUDAQ_GIT_REPOSITORY "https://github.com/NVIDIA/cuda-quantum.git"
    CACHE STRING "CUDA-Q git repository")
set(CUDAQ_GIT_TAG "0.15.0" CACHE STRING "CUDA-Q v0.15.0 build")

if(CUDAQ_AUTO_FETCH AND NOT CUDAQ_ROOT AND NOT DEFINED ENV{CUDAQ_INSTALL_PREFIX})
  ...
  # CUDA-Q must be built against the same LLVM this project uses, so
  # require that find_package(MLIR CONFIG) already ran.
  ...
  set(_cudaq_marker "${_cudaq_install}/include/cudaq/Optimizer/Dialect/Quake/QuakeOps.h")
  if(NOT EXISTS "${_cudaq_marker}")
    ...
    if(NOT EXISTS "${_cudaq_src}/.git")
      execute_process(
        COMMAND git clone --depth 1 --branch ${CUDAQ_GIT_TAG}
                ${CUDAQ_GIT_REPOSITORY} ${_cudaq_src}
    ...
    string(REPLACE "    add_subdirectory(nvqpp)" "#    add_subdirectory(nvqpp) -- patched out by FindCUDAQ.cmake"
    ...
    execute_process(
      COMMAND ${CMAKE_COMMAND}
              -G ${CMAKE_GENERATOR}
              -S ${_cudaq_src} -B ${_cudaq_build}
              -DCMAKE_INSTALL_PREFIX=${_cudaq_install}
              -DCMAKE_BUILD_TYPE=Release
              -DLLVM_DIR=${LLVM_DIR}
              -DMLIR_DIR=${MLIR_DIR}
              -DCUDAQ_ENABLE_PYTHON=OFF
              -DCUDAQ_BUILD_TESTS=OFF
              -DCUDAQ_DISABLE_RUNTIME=ON
      RESULT_VARIABLE _cudaq_rc)
    if(NOT _cudaq_rc EQUAL 0)
    ...
    set(CUDAQ_BUILD_PARALLELISM "${_cudaq_nproc}" CACHE STRING
        "Parallel build jobs for the CUDA-Q auto-fetch build (lower this if cc1plus gets OOM-killed)")

    execute_process(
      COMMAND ${CMAKE_COMMAND} --build ${_cudaq_build}
              --target QuakeDialect CCDialect QECDialect OptimBuilder OptCodeGen
              -- -j${CUDAQ_BUILD_PARALLELISM}
      RESULT_VARIABLE _cudaq_rc)
    if(NOT _cudaq_rc EQUAL 0)
      message(FATAL_ERROR "FindCUDAQ: build failed (exit ${_cudaq_rc})")
    endif()
    execute_process(
    COMMAND ${CMAKE_COMMAND} --install ${_cudaq_build} --component QuakeDialect
    RESULT_VARIABLE _cudaq_rc)
    if(_cudaq_rc EQUAL 0)
        execute_process(
            COMMAND ${CMAKE_COMMAND} --install ${_cudaq_build} --component CCDialect
        RESULT_VARIABLE _cudaq_rc)
    endif()
    if(_cudaq_rc EQUAL 0)
        execute_process(
            COMMAND ${CMAKE_COMMAND} --install ${_cudaq_build} --component QECDialect
        RESULT_VARIABLE _cudaq_rc)
    endif()
    if(_cudaq_rc EQUAL 0)
        execute_process(
            COMMAND ${CMAKE_COMMAND} --install ${_cudaq_build} --component OptimBuilder
        RESULT_VARIABLE _cudaq_rc)
    endif()
    if(_cudaq_rc EQUAL 0)
        execute_process(
            COMMAND ${CMAKE_COMMAND} --install ${_cudaq_build} --component OptCodeGen
        RESULT_VARIABLE _cudaq_rc)
    endif()
    if(NOT _cudaq_rc EQUAL 0)
...
find_path(CUDAQ_GENERATED_INCLUDE_DIR
...
find_package_handle_standard_args(CUDAQ
  REQUIRED_VARS
    CUDAQ_INCLUDE_DIR
...
if(CUDAQ_FOUND)
  ...
  if(NOT TARGET CUDAQ::CC)
    ...
    set_target_properties(CUDAQ::CC PROPERTIES
  ...
  if(NOT TARGET CUDAQ::Quake)
    ...
    set_target_properties(CUDAQ::Quake PROPERTIES
      IMPORTED_LOCATION "${CUDAQ_QUAKE_LIBRARY}"
      INTERFACE_INCLUDE_DIRECTORIES "${CUDAQ_INCLUDE_DIRS}"
      INTERFACE_LINK_LIBRARIES
        "CUDAQ::CC;MLIRIR;MLIRFuncDialect")
  endif()

  if(NOT TARGET CUDAQ::QEC)
    add_library(CUDAQ::QEC UNKNOWN IMPORTED GLOBAL)
    set_target_properties(CUDAQ::QEC PROPERTIES
      IMPORTED_LOCATION "${CUDAQ_QEC_LIBRARY}"
      INTERFACE_INCLUDE_DIRECTORIES "${CUDAQ_INCLUDE_DIRS}"
      INTERFACE_LINK_LIBRARIES
        "CUDAQ::Quake;CUDAQ::CC;MLIRIR")
  endif()

  if(CUDAQ_CODEGEN_LIBRARY AND NOT TARGET CUDAQ::CodeGen)
    add_library(CUDAQ::CodeGen UNKNOWN IMPORTED GLOBAL)
    set_target_properties(CUDAQ::CodeGen PROPERTIES
  ...
  if(CUDAQ_BUILDER_LIBRARY AND NOT TARGET CUDAQ::Builder)
    ...
    set_target_properties(CUDAQ::Builder PROPERTIES
