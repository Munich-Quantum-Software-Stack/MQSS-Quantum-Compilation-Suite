...
private:
   ...
   * libraries that are subclasses of qdmi_main_driver::DeviceLibrary.
   */
  std::unique_ptr<qdmi_main_driver::DeviceLibrary> library_;
...
public:
   ...
   */
  explicit QDMI_Device_impl_d(std::unique_ptr<qdmi_main_driver::DeviceLibrary>&& lib,
  ...
  [[nodiscard]] auto getLibrary() const -> const qdmi_main_driver::DeviceLibrary& {
    return *library_;
  }
...
private:
  ...
  qdmi_main_driver::SessionStatus status_ = qdmi_main_driver::SessionStatus::ALLOCATED;
