                        ...
                        ValueRange inQubits)
{
    ...
    LLVM::LLVMFuncOp fnDecl =
        ensureFunctionDeclaration<LLVM::LLVMFuncOp>(rewriter, op, qirName, qirSignature);
  ...
  protected:
                         ...
                         T op, typename T::Adaptor adaptor) const
    {
        ...
        if constexpr (std::is_same_v<T, SampleOp>) {
            ...
            auto cStruct = LLVM::InsertValueOp::create(
                rewriter, loc, bStruct, adaptor.getInCounts(), SmallVector<int64_t>{1});
