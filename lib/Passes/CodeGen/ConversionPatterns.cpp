...
#include "Passes/CodeGen/StaticAllocas.h"
...
OpT ensureFunctionDeclaration(PatternRewriter &rewriter, Operation *op,
                              StringRef fnSymbol, TypeT fnType) {
  ...
  if (!fnDecl) {
    ...
    // func::FuncOp usually requires explicit private visibility for runtime
    // decls.
    if (isa<mlir::func::FuncOp>(newFunc)) {
      newFunc.setPrivate();
    }
    ...
    assert(isa<OpT>(fnDecl) &&
           "Existing symbol is not the expected operation type");
  }

  return cast<OpT>(fnDecl);
}

Value getGlobalString(Location loc, OpBuilder &rewriter, StringRef key,
                      StringRef value, ModuleOp mod) {
  auto type = LLVM::LLVMArrayType::get(
      IntegerType::get(rewriter.getContext(), 8), value.size());
  LLVM::GlobalOp glb = mod.lookupSymbol<LLVM::GlobalOp>(key);
  if (!glb) {
    OpBuilder::InsertionGuard guard(rewriter); // to reset the insertion point
    rewriter.setInsertionPointToStart(mod.getBody());
    glb = LLVM::GlobalOp::create(rewriter, loc, type, true,
                                 LLVM::Linkage::Internal, key,
                                 rewriter.getStringAttr(value));
  }
  return LLVM::GEPOp::create(
      rewriter, loc, LLVM::LLVMPointerType::get(rewriter.getContext()), type,
      LLVM::AddressOfOp::create(rewriter, loc, glb),
      ArrayRef<LLVM::GEPArg>{0, 0}, LLVM::GEPNoWrapFlags::inbounds);
}

Value getPauliProductPtr(Location loc, OpBuilder &rewriter, ModuleOp mod,
  ...
  return getGlobalString(loc, rewriter, pauliWordKey,
                         StringRef(pauliWord.c_str(), pauliWord.length() + 1),
                         mod);
}

void createPauliRotCall(Location loc, ConversionPatternRewriter &rewriter,
                        Operation *op, Value pauliWordPtr, Value thetaValue,
                        Value modifiersPtr, Value cond, ValueRange inQubits) {
  MLIRContext *ctx = rewriter.getContext();
  StringRef qirName = "__catalyst__qis__PauliRot";
  Type ptrType = LLVM::LLVMPointerType::get(ctx);
  Type qirSignature = LLVM::LLVMFunctionType::get(
      LLVM::LLVMVoidType::get(ctx),
      {ptrType, Float64Type::get(ctx), ptrType, IntegerType::get(ctx, 1),
       IntegerType::get(ctx, 64)},
      /*isVarArg=*/true);

  LLVM::LLVMFuncOp fnDecl = ensureFunctionDeclaration<LLVM::LLVMFuncOp>(
  ...
  args.push_back(cond);
  args.push_back(LLVM::ConstantOp::create(
...
/**
 * @brief Initialize and fill the  `struct Modifiers` on stack return a pointer
 * to it.
 *
 ...
 */
Value getModifiersPtr(Location loc, RewriterBase &rewriter,
                      const TypeConverter *conv, bool adjoint,
  ...
  auto adjointVal =
      LLVM::ConstantOp::create(rewriter, loc, rewriter.getBoolAttr(adjoint));
  auto structType = LLVM::LLVMStructType::getLiteral(
      ctx, {boolType, sizeType, ptrType, ptrType});
  auto modifiersPtr =
      catalyst::getStaticAlloca(loc, rewriter, structType, 1).getResult();
  auto adjointPtr = LLVM::GEPOp::create(
      rewriter, loc, ptrType, structType, modifiersPtr,
      llvm::ArrayRef<LLVM::GEPArg>{0, 0}, LLVM::GEPNoWrapFlags::inbounds);
  auto numControlledPtr = LLVM::GEPOp::create(
      rewriter, loc, ptrType, structType, modifiersPtr,
      llvm::ArrayRef<LLVM::GEPArg>{0, 1}, LLVM::GEPNoWrapFlags::inbounds);
  auto controlledWiresPtr = LLVM::GEPOp::create(
      rewriter, loc, ptrType, structType, modifiersPtr,
      llvm::ArrayRef<LLVM::GEPArg>{0, 2}, LLVM::GEPNoWrapFlags::inbounds);
  auto controlledValuesPtr = LLVM::GEPOp::create(
      rewriter, loc, ptrType, structType, modifiersPtr,
  ...
  if (!controlledQubits.empty()) {
    ctrlPtr = catalyst::getStaticAlloca(loc, rewriter, ptrType,
                                        controlledQubits.size())
                  .getResult();
    valuePtr = catalyst::getStaticAlloca(loc, rewriter, boolType,
                                         controlledQubits.size())
                   .getResult();
    for (int i = 0; static_cast<size_t>(i) < controlledQubits.size(); i++) {
      {
        auto itemPtr = LLVM::GEPOp::create(
            rewriter, loc, ptrType, ptrType, ctrlPtr,
            llvm::ArrayRef<LLVM::GEPArg>{i}, LLVM::GEPNoWrapFlags::inbounds);
        auto qubit = controlledQubits[i];
        LLVM::StoreOp::create(rewriter, loc, qubit, itemPtr);
      }
      {
        auto itemPtr = LLVM::GEPOp::create(
            rewriter, loc, ptrType, boolType, valuePtr,
            llvm::ArrayRef<LLVM::GEPArg>{i}, LLVM::GEPNoWrapFlags::inbounds);
  ...
  auto ctrlQubits = LLVM::ConstantOp::create(
      rewriter, loc, rewriter.getI64IntegerAttr(controlledQubits.size()));
    ...
    if constexpr (std::is_same_v<T, InitializeOp>) {
      ...
      if (op->hasAttr("seed")) {
        ...
        OpBuilder::InsertionGuard guard(
            rewriter); // to reset the insertion point
        rewriter.setInsertionPointToStart(mod.getBody());
        LLVM::GlobalOp seed_glb =
            LLVM::GlobalOp::create(rewriter, loc, IntegerType::get(ctx, 32),
                                   true, LLVM::Linkage::Internal, "seed",
...
struct DeviceInitOpPattern : public OpConversionPattern<DeviceInitOp> {
  ...
  LogicalResult
  matchAndRewrite(DeviceInitOp op, DeviceInitOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op.getLoc();
    MLIRContext *ctx = this->getContext();
    ModuleOp mod = op->getParentOfType<ModuleOp>();
    ...
    StringRef qirName =
        "__catalyst__rt__device_init"; // (int8_t *, int8_t *, int8_t *) -> void
    ...
    Type qirSignature = LLVM::LLVMFunctionType::get(
        LLVM::LLVMVoidType::get(ctx), {/* rtd_lib = */ charPtrType,
    ...
    if (!shots) {
      auto zeroShots = LLVM::ConstantOp::create(rewriter, loc,
    ...
    Value autoQubitManagement = LLVM::ConstantOp::create(
        rewriter, loc, rewriter.getI1Type(), op.getAutoQubitManagement());
...
struct DeviceReleaseOpPattern : public OpConversionPattern<DeviceReleaseOp> {
  ...
  LogicalResult
  matchAndRewrite(DeviceReleaseOp op, DeviceReleaseOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    MLIRContext *ctx = this->getContext();
...
struct NumQubitsOpPattern : public OpConversionPattern<NumQubitsOp> {
  ...
  LogicalResult
  matchAndRewrite(NumQubitsOp op, NumQubitsOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    MLIRContext *ctx = this->getContext();
...
struct AllocOpPattern : public OpConversionPattern<AllocOp> {
  ...
  matchAndRewrite(AllocOp op, AllocOpAdaptor adaptor,
    ...
    if (!nQubits) {
      nQubits =
          LLVM::ConstantOp::create(rewriter, loc, op.getNqubitsAttrAttr());
    }
...
struct AllocQubitOpPattern : public OpConversionPattern<AllocQubitOp> {
  ...
  LogicalResult
  matchAndRewrite(AllocQubitOp op, AllocQubitOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    MLIRContext *ctx = getContext();
    const TypeConverter *conv = getTypeConverter();
    ...
    Type qirSignature =
        LLVM::LLVMFunctionType::get(conv->convertType(QubitType::get(ctx)), {});
...
struct DeallocOpPattern : public OpConversionPattern<DeallocOp> {
  ...
  matchAndRewrite(DeallocOp op, DeallocOpAdaptor adaptor,
    ...
    Type qirSignature = LLVM::LLVMFunctionType::get(
        LLVM::LLVMVoidType::get(ctx), conv->convertType(QuregType::get(ctx)));
...
struct DeallocQubitOpPattern : public OpConversionPattern<DeallocQubitOp> {
  ...
  LogicalResult
  matchAndRewrite(DeallocQubitOp op, DeallocQubitOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    MLIRContext *ctx = getContext();
    const TypeConverter *conv = getTypeConverter();
    ...
    Type qirSignature = LLVM::LLVMFunctionType::get(
        LLVM::LLVMVoidType::get(ctx), conv->convertType(QubitType::get(ctx)));
...
struct ExtractOpPattern : public OpConversionPattern<ExtractOp> {
  ...
  LogicalResult
  matchAndRewrite(ExtractOp op, ExtractOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op.getLoc();
    MLIRContext *ctx = getContext();
    const TypeConverter *conv = getTypeConverter();
    ...
    Value elemPtr =
        LLVM::CallOp::create(rewriter, loc, fnDecl, operands).getResult();
    rewriter.replaceOpWithNewOp<LLVM::LoadOp>(
        op, conv->convertType(QubitType::get(ctx)), elemPtr);
...
struct InsertOpDefaultPattern : public OpConversionPattern<InsertOp> {
  ...
  LogicalResult
  matchAndRewrite(InsertOp op, InsertOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    // Unravel use-def chain of quantum register values, converting back to
    // reference semantics.
...
struct CustomOpPattern : public OpConversionPattern<CustomOp> {
  ...
  LogicalResult
  matchAndRewrite(CustomOp op, CustomOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op.getLoc();
    MLIRContext *ctx = getContext();
    ...
    const TypeConverter *conv = getTypeConverter();
    auto modifiersPtr =
        getModifiersPtr(loc, rewriter, conv, op.getAdjointFlag(),
                        adaptor.getInCtrlQubits(), adaptor.getInCtrlValues());

    // TODO: Might be better to have corresponding attribute for those custom op
    // with variadic input qubits, so we don't need to explicitly check any op
    // gate name here
    ...
    SmallVector<Value> args;
    args.insert(args.end(), adaptor.getParams().begin(),
                adaptor.getParams().end());
    if (hasVariadicInputQbits) {
      // get the number of qbuits and place the input qubits at the end of the
      // arguments.
      ...
      args.insert(args.end(),
                  LLVM::ConstantOp::create(
                      rewriter, loc, rewriter.getI64IntegerAttr(numQubits)));
      args.insert(args.end(), adaptor.getInQubits().begin(),
                  adaptor.getInQubits().end());
    } else {
      args.insert(args.end(), adaptor.getInQubits().begin(),
                  adaptor.getInQubits().end());
      args.insert(args.end(), modifiersPtr);
    ...
    values.insert(values.end(), adaptor.getInQubits().begin(),
                  adaptor.getInQubits().end());
...
struct GlobalPhaseOpPattern : public OpConversionPattern<GlobalPhaseOp> {
  ...
  LogicalResult
  matchAndRewrite(GlobalPhaseOp op, GlobalPhaseOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op.getLoc();
    MLIRContext *ctx = getContext();
    const TypeConverter *conv = getTypeConverter();
    ...
    Type qirSignature = LLVM::LLVMFunctionType::get(
        LLVM::LLVMVoidType::get(ctx),
        {Float64Type::get(ctx), modifiersPtr.getType()});
...
struct MultiRZOpPattern : public OpConversionPattern<MultiRZOp> {
  ...
  LogicalResult
  matchAndRewrite(MultiRZOp op, MultiRZOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op.getLoc();
    MLIRContext *ctx = getContext();
    const TypeConverter *conv = getTypeConverter();
...
struct PauliRotOpPattern : public OpConversionPattern<PauliRotOp> {
  ...
  LogicalResult
  matchAndRewrite(PauliRotOp op, PauliRotOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op.getLoc();
    const TypeConverter *conv = getTypeConverter();
    ModuleOp mod = op->getParentOfType<ModuleOp>();
    ...
    Value pauliWordPtr =
        getPauliProductPtr(loc, rewriter, mod, op.getPauliProduct());
    Value thetaValue = adaptor.getAngle();
    Value cond =
        LLVM::ConstantOp::create(rewriter, loc, rewriter.getBoolAttr(true));

    createPauliRotCall(loc, rewriter, op.getOperation(), pauliWordPtr,
                       thetaValue, modifiersPtr, cond, adaptor.getInQubits());
...
struct PCPhaseOpPattern : public OpConversionPattern<PCPhaseOp> {
  ...
  LogicalResult
  matchAndRewrite(PCPhaseOp op, PCPhaseOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op.getLoc();
    MLIRContext *ctx = getContext();
    const TypeConverter *conv = getTypeConverter();
    ...
    Type qirSignature = LLVM::LLVMFunctionType::get(
        LLVM::LLVMVoidType::get(ctx),
        {Float64Type::get(ctx), Float64Type::get(ctx), modifiersPtr.getType(),
         IntegerType::get(ctx, 64)},
        /*isVarArg=*/true);
...
struct QubitUnitaryOpPattern : public OpConversionPattern<QubitUnitaryOp> {
  ...
  LogicalResult
  matchAndRewrite(QubitUnitaryOp op, QubitUnitaryOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op.getLoc();
    MLIRContext *ctx = getContext();
    const TypeConverter *conv = getTypeConverter();
    ...
    Type matrixType = conv->convertType(MemRefType::get(
        {UNKNOWN, UNKNOWN}, ComplexType::get(Float64Type::get(ctx))));
...
struct ComputationalBasisOpPattern
    : public OpConversionPattern<ComputationalBasisOp> {
  using OpConversionPattern::OpConversionPattern;

  LogicalResult
  matchAndRewrite(ComputationalBasisOp op, ComputationalBasisOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    ...
    // Therefore, for the qubit case, let the unrealized cast op carry the list
    // of qubits and for qreg case, let the unrealized cast op carry no
    // arguments
...
struct NamedObsOpPattern : public OpConversionPattern<NamedObsOp> {
  ...
  LogicalResult
  matchAndRewrite(NamedObsOp op, NamedObsOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op.getLoc();
    MLIRContext *ctx = getContext();
    const TypeConverter *conv = getTypeConverter();
    ...
    Value obsType = LLVM::ConstantOp::create(
        rewriter, loc, rewriter.getI64IntegerAttr(obsTypeInt));
...
struct HermitianOpPattern : public OpConversionPattern<HermitianOp> {
  ...
  LogicalResult
  matchAndRewrite(HermitianOp op, HermitianOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op.getLoc();
    MLIRContext *ctx = getContext();
    const TypeConverter *conv = getTypeConverter();
...
struct TensorOpPattern : public OpConversionPattern<TensorOp> {
  ...
  LogicalResult
  matchAndRewrite(TensorOp op, TensorOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op.getLoc();
    MLIRContext *ctx = getContext();
    const TypeConverter *conv = getTypeConverter();
    ...
    Type qirSignature = LLVM::LLVMFunctionType::get(
        conv->convertType(ObservableType::get(ctx)), IntegerType::get(ctx, 64),
        /*isVarArg=*/true);
    ...
    args.insert(args.begin(),
                LLVM::ConstantOp::create(rewriter, loc,
                                         rewriter.getI64IntegerAttr(numTerms)));
...
struct HamiltonianOpPattern : public OpConversionPattern<HamiltonianOp> {
  ...
  LogicalResult
  matchAndRewrite(HamiltonianOp op, HamiltonianOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op.getLoc();
    MLIRContext *ctx = getContext();
    const TypeConverter *conv = getTypeConverter();
    ...
    Type vectorType =
        conv->convertType(MemRefType::get({UNKNOWN}, Float64Type::get(ctx)));
...
struct MeasureOpPattern : public OpConversionPattern<MeasureOp> {
  ...
  LogicalResult
  matchAndRewrite(MeasureOp op, MeasureOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op.getLoc();
    MLIRContext *ctx = getContext();
    const TypeConverter *conv = getTypeConverter();
    ...
    Type qirSignature = LLVM::LLVMFunctionType::get(
        conv->convertType(ResultType::get(ctx)), argSignatures);
    ...
    Value resultPtr =
        LLVM::CallOp::create(rewriter, loc, fnDecl, args).getResult();
    Value mres = LLVM::LoadOp::create(rewriter, loc, IntegerType::get(ctx, 1),
                                      resultPtr);
...
protected:
  Value performRewrite(ConversionPatternRewriter &rewriter, Type structType,
                       StringRef qirName, T op,
                       typename T::Adaptor adaptor) const {
    Location loc = op.getLoc();
    MLIRContext *ctx = this->getContext();
    ...
    // ComputationalBasisOp lowering. Improve this once the runtime interface
    // changes to accept observables for sample.
    ...
    Value numQubits = LLVM::ConstantOp::create(
        rewriter, loc, rewriter.getI64IntegerAttr(qubits.size()));
    ...
    if constexpr (std::is_same_v<T, SampleOp>) {
      ...
      auto bStruct = LLVM::InsertValueOp::create(rewriter, loc, aStruct,
                                                 adaptor.getInEigvals(),
                                                 SmallVector<int64_t>{0});
      auto cStruct = LLVM::InsertValueOp::create(rewriter, loc, bStruct,
                                                 adaptor.getInCounts(),
...
struct SampleOpPattern : public SampleBasedPattern<SampleOp> {
  ...
  LogicalResult
  matchAndRewrite(SampleOp op, SampleOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    MLIRContext *ctx = getContext();
    const TypeConverter *conv = getTypeConverter();
    ...
    Type matrixType = conv->convertType(
        MemRefType::get({UNKNOWN, UNKNOWN}, Float64Type::get(ctx)));
...
struct CountsOpPattern : public SampleBasedPattern<CountsOp> {
  ...
  LogicalResult
  matchAndRewrite(CountsOp op, CountsOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    MLIRContext *ctx = getContext();
    const TypeConverter *conv = getTypeConverter();
    ...
    Type vector1Type =
        conv->convertType(MemRefType::get({UNKNOWN}, Float64Type::get(ctx)));
    Type vector2Type = conv->convertType(
        MemRefType::get({UNKNOWN}, IntegerType::get(ctx, 64)));
    Type structType =
        LLVM::LLVMStructType::getLiteral(ctx, {vector1Type, vector2Type});
...
template <typename T> struct StateBasedPattern : public OpConversionPattern<T> {
  ...
  matchAndRewrite(T op, typename T::Adaptor adaptor,
    ...
    if constexpr (std::is_same_v<T, ProbsOp>) {
      ...
      // __catalyst__qis__State does not support individual qubit measurements
      // yet, so it must be invoked without specific specific qubits (i.e.
      // measure the whole register).
      Value numQubits = LLVM::ConstantOp::create(rewriter, loc,
                                                 rewriter.getI64IntegerAttr(0));
      args.push_back(numQubits);
    }
...
struct SetStateOpPattern : public OpConversionPattern<SetStateOp> {
  ...
  LogicalResult
  matchAndRewrite(SetStateOp op, SetStateOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    bool isVarArg = true;
    MLIRContext *ctx = rewriter.getContext();
    ...
    auto func = mlir::LLVM::lookupOrCreateFn(rewriter, moduleOp,
                                             "__catalyst__qis__SetState",
    ...
    Value c = LLVM::ConstantOp::create(rewriter, loc,
...
struct SetBasisStateOpPattern : public OpConversionPattern<SetBasisStateOp> {
  ...
  LogicalResult
  matchAndRewrite(SetBasisStateOp op, SetBasisStateOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    bool isVarArg = true;
    MLIRContext *ctx = rewriter.getContext();
    auto i64 = IntegerType::get(ctx, 64);
    ...
    ModuleOp moduleOp = op->getParentOfType<ModuleOp>();
    auto func = mlir::LLVM::lookupOrCreateFn(rewriter, moduleOp,
                                             "__catalyst__qis__SetBasisState",
                                             {ptrTy, i64}, voidTy, isVarArg)
                    .value();
...
void populateQIRConversionPatterns(TypeConverter &typeConverter,
                                   RewritePatternSet &patterns,
                                   bool useArrayBackedRegisters) {
  patterns.add<RTBasedPattern<InitializeOp>>(typeConverter,
                                             patterns.getContext());
  patterns.add<RTBasedPattern<FinalizeOp>>(typeConverter,
                                           patterns.getContext());
  ...
  if (useArrayBackedRegisters) {
    patterns.add<InsertOpArrayBackedPattern>(typeConverter,
                                             patterns.getContext());
  } else {
  ...
  patterns.add<ComputationalBasisOpPattern>(typeConverter,
                                            patterns.getContext());
  ...
  patterns.add<StatsBasedPattern<ExpvalOp>>(typeConverter,
                                            patterns.getContext());
  patterns.add<StatsBasedPattern<VarianceOp>>(typeConverter,
                                              patterns.getContext());
  patterns.add<StateBasedPattern<ProbsOp>>(typeConverter,
                                           patterns.getContext());
  patterns.add<StateBasedPattern<StateOp>>(typeConverter,
                                           patterns.getContext());
