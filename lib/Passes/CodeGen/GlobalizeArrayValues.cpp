...
SmallVector<A> conversion(ArrayAttr seq, mlir::Type) {
...
conversion<std::complex<APFloat>, ArrayAttr>(ArrayAttr seq, mlir::Type) {
...
convertArrayAttrToGlobalConstant(MLIRContext *ctx, Location loc,
                                 ...
                                 StringRef globalName, mlir::Type eleTy) {
  ...
  if (isa<ComplexType>(eleTy)) {
    ...
    auto dense = DenseElementsAttr::get(tensorTy, blockValues);
    irBuilder.genVectorOfConstants(loc, module, globalName, dense, eleTy);
  } else if (isa<mlir::IntegerType>(eleTy)) {
...
static bool multidimensionalArray(mlir::Type ty) {
  ...
  Value buildSpans(Location loc, cudaq::cc::SpanLikeType ty,
    ...
    for (auto attr : arrAttr) {
      if (auto a = dyn_cast<ArrayAttr>(attr)) {
        ...
        auto litTy = cudaq::cc::PointerType::get(
    ...
    if (auto iTy = dyn_cast<mlir::IntegerType>(eleTy)) {
      if (iTy.getWidth() == 1) {
        eleTy = mlir::IntegerType::get(ty.getContext(), 8);
        hasBoolElems = true;
      }
    }
...
/// `.rodata` section.
struct GlobalizeArrayValuesPass
...
// std::unique_ptr<mlir::Pass> cudaq::opt::createGlobalizeArrayValues() {
//   return std::make_unique<GlobalizeArrayValuesPass>();
// }